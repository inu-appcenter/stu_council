...
export default {
  ...
  methods: {
    ...
    getContentsList(){
          var self = this
          axios.post(`${global.base}/board/all`, {boardKind: 4})
          .then(response =>{
            var financialData = response.data[0]
            console.log(financialData)
              for(var item in financialData){
                var content = new Object()

                content.index = parseInt(item) + 1
                content.title = financialData[item].title
                content.author = financialData[item].author
                content.date = self.getDate(financialData[item].date)
                content.viewTime = financialData[item].viewTime
                content.boardId = financialData[item].boardId
