<template>
  <div id="login" class="container">
    <div class="login__wrapper">
      ...
      <input
        class="login__wrapper-input"
        v-on:keyup.enter="bt_listener()"
        type="text"
        v-model.trim="id"
        placeholder="학번"
      >
...
export default {
  ...
  methods: {
    ...
    go_login(id, password) {
      ...
      axios
        .post(`${base}/login/signIn`, { id: self.id, passwd: self.password })
        .then(response => {
          self.$session.start();
          self.$session.set("member_id", self.id);
